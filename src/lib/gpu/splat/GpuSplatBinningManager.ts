...
export class GpuSplatBinningManager {
    ...
    constructor({
        ...
        const binningModule = device.createShaderModule({
            ...
            code: injectWgslConstants(binningModuleSrc, {
                ...
                NUM_SPLATS_PLUS_ONE: numSplats + 1,
                NUM_SPLATS_MINUS_ONE: numSplats - 1,
                NUM_SPLATS_DIV_32: Math.ceil(numSplats / 32),
    ...
    addDispatches(pass: GPUComputePassEncoder, vpMat: Mat4, width: number, height: number) {
        ...
        pass.dispatchWorkgroups(Math.ceil(this.numSplats / 256));
